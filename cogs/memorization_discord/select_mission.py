...
class SelectMissionView(discord.ui.View):
    def __init__(self,titles,missions:list,mode):
        super().__init__()
        ...
        self.add_item(SelectMission_1(self.options_1,self.mode))
        if missions_len > 25:
            self.add_item(SelectMission_2(self.options_2,self.mode))
        if missions_len > 50:
            self.add_item(SelectMission_3(self.options_3,self.mode))
        if missions_len > 75:
            self.add_item(SelectMission_4(self.options_4,self.mode))
...
class SelectMiView:
    ...
    async def select_response(self):
        if self.mode == 0:
            await Delete().delete_misson_select(str(self.intracton.user.id),self.selectnumber)
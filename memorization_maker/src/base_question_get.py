...
class Get:
    ...
    async def get_misson_select(self,user_id:str,title:str,select_len_number:int):
        ...
        if not await self.owner.ower_check(num_id,title):return False
    ...
    async def check_answer(self,user_id:str,title:str,select_len_number:int,answer:int,text_question_number:int = None):
        num_id = str(user_id)
        self.base_data:dict = await self.rw.load_base()
        sharecode = await self.share.get_sharecode(user_id,title)
        if not await self.owner.ower_check(num_id,title):return False
    ...
    async def get_anwser(self,user_id:str,title:str,select_len_number:int):
        num_id = str(user_id)
        self.base_data:dict = await self.rw.load_base()
        sharecode = await self.share.get_sharecode(user_id,title)
        if not await self.owner.ower_check(num_id,title):return False

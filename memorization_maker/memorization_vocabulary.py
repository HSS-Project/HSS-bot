...
class Vocabulary:
    ...
    async def make_vocabulary(self,user_id:str,title:str,start_number:int,end_unmber:int,mode:int = 0):
        num_id = str(user_id)
        ...
        list_data = await self.get_question_list_from_excel(start_number,end_unmber,mode)
        for data in list_data:await self.make_add.add_misson_select(sharecode,data["question"],data["answer"],data["select"])
        return sharecode

    async def get_question_list_from_excel(self,start_number, end_number,mode:int):
        ...
        for row in range(start_number + 2, end_number + 3):
            ...
            if mode == 0:
                ...
                quesiton = sheet[f'E{row}'].value
                answer = sheet[f'D{row}'].value
                select = [sheet[f'D{x}'].value for x in number_list]
